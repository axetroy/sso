...
func file(writer http.ResponseWriter) (err error) {
  var (
    stat os.FileInfo
    file *os.File
  )
...
func folder(writer http.ResponseWriter) (err error) {
  var (
    file *os.File
  )
  z := archiver.Tar{
...
func writeFile(rootDir, baseDir string, z archiver.Tar) (err error) {
  ...
  for _, fileInfo := range files {
    ...
    internalName = rootDir + "/" + internalName
    ...
    // 如果是目录的话，继续便利下一层目录
    if fileInfo.IsDir() {
      err = writeFile(rootDir+"/"+filename, path.Join(baseDir, filename), z)
    }
  }
